...
<body>
			...
			<?php
                ...
                if(!isset($_POST['submit'])) { // if there is no answers[] array that means the form hasn't been submitted, display the form
                    ...
                    echo "<form action=\"zodiacTextQuiz.php\" method=\"post\">\n";
                    ...
                    if(isset($incomplete)) { // begin error checking
                        ...
                        $quizTaken = $quizTaken + 1; // add one to the number of quizzes taken
                        $fileString = $quizTaken."\n"; // add the new total of quizzes to the string that will be written to the file, need a newline
                        
						// implode our question stats array so it's ready for storage as values separated by commas
                        $qStats = implode(",", $qStats);
						
						// add the question stats to the string that will be written to the file, remember the newline
                        $fileString .= $qStats."\n";
						
						// implode the grade stats and add them to the stringthat will be written to the file
                        $gradeStats = implode(",", $gradeStats);
                        $fileString .= $gradeStats; // no newline this time
						
						// write the file string to the file for storage
                        ...
                        $name = $_POST['name'];
                        $email = $_POST['email'];
                        $to = "alamhain@roadrunner.com";
                        $subject = "Chinese Zodiac Quiz Response";
                        $headers = "From: $name <$email>";
						
						// add the score info to the start of the email
						
						$scoreString = "$name got $correct right out of 10, scoring $score\n\n";
						$message = substr_replace($message, $scoreString, 0, 0);
                        
						// send mail to the quiz reviewer     
